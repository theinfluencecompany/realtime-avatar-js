...
test("a state map compiles into clip_library with the cue as the hint", async () => {
  const { seen, fetchImpl } = stub({ body: GRANT });
  const rta = new RealtimeAvatar({ apiKey: "k", fetch: fetchImpl });
  await rta.startCall({
    avatarId: "ava_1",
  ...
  assert.deepEqual(seen.body?.clip_library, [{
    ...
    trigger: "directive", hint: "when the user is happy", weight: 0.3,
  }]);
});

test("a single loop sets BOTH source keys, or the call is rejected", async () => {
  const { seen, fetchImpl } = stub({ body: GRANT });
  const rta = new RealtimeAvatar({ apiKey: "k", fetch: fetchImpl });
  await rta.startCall({ avatarId: "ava_1", video: { loop: "https://x/idle.mp4" } });
  assert.equal(seen.body?.source_kind, "source_video");
  assert.equal(seen.body?.source_video_url, "https://x/idle.mp4");
});
