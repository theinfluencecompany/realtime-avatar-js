...
export interface RealtimeAvatarOptions {
   ...
   *
   * Set 0 to disable. Every retried write carries the same `Idempotency-Key`, so a retry
   * cannot double-charge you for a call the first attempt actually started.
   */
  maxRetries?: number;
...
function videoToWire(video: NonNullable<CallPolicy["video"]>): Record<string, unknown> {
  ...
  if (video.loop) {
    // Both keys, or the call is rejected.
    out.source_kind = "source_video";
    out.source_video_url = video.loop;
  }
  if (video.states) {
